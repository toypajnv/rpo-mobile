...
class DashboardCacheBustTests(unittest.TestCase):
    def test_dashboard_loader_revision_forces_fresh_operator_controls(self) -> None:
        self.assertEqual(main.DASHBOARD_ASSET_VERSION, "20260909-1")
        source, _, _ = main._core.templates.env.loader.get_source(
            main._core.templates.env, "dashboard.html"
        )
        self.assertIn('/static/dashboard.js?v=20260909-1', source)
        self.assertIn('/static/dashboard-decisions.js?v=20260909-1', source)
    ...
    def test_decision_controls_reapply_after_both_tables_refresh(self) -> None:
        decisions = (main._core.BASE_DIR / "static" / "dashboard-decisions.js").read_text(encoding="utf-8")
        ...
        self.assertIn("new MutationObserver(() => queueMicrotask(annotateTransmissions))", decisions)
        self.assertIn("observe(worksBody, {childList:true})", decisions)
        self.assertIn("observe(transmissionsBody, {childList:true})", decisions)
